...
/*   Updated: 2024/08/18 13:19:12 by mamazari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_cd(t_args *args, char **av)
{
	...
	{
		prev_pwd = ft_strdup(get_value_from_key(&args->export_list, "OLDPWD"));
		if (!prev_pwd)
			prev_pwd = ft_strdup(get_value_from_key(&args->export_list, "HOME"));
	}
	free(path);
	if (!av[1])
